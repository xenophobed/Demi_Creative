...
#### 待完成 (Remaining Gaps)
- 🔲 **安全分数提取**: `_generate_with_sdk()` 硬编码 `safety_score=0.9`，未从 SDK 响应提取真实安全分数
- 🔲 **SDK 路径契约测试**: 仅 mock 路径有测试，SDK 响应解析/归一化/嘉宾注入无测试覆盖
- 🔲 **显式 Mock 环境标志**: 缺少 `MORNING_SHOW_FORCE_MOCK` 环境变量，当前仅依赖 pytest 检测
- 🔲 **角色命名**: 验收标准要求 Duo + Mimi 角色名，当前代码使用 `curious_kid` / `fun_expert`
