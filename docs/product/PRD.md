...
##### 功能描述
儿童无需等待次日的 Daily Drop，可以随时按下"立即收听"按钮，系统实时获取最新新闻并生成个性化播客节目。这将"早间秀"升级为"随时听"(Kids Daily)，降低使用门槛，让儿童在任何想听新闻的时刻都能立即获得内容。
...
- 新闻中心 Morning Show 模式：频道卡片同样提供"立即收听"操作
- 生成完成后自动跳转播放器页面
...
##### 功能描述
前端将"手动新闻转换"和"早间秀"合并为单一入口"儿童新闻"(Kids News)，提供两种模式切换。首页导航、创作库标签页均统一为一个入口，降低儿童认知负担。
...
进入新闻中心页面，顶部有模式切换：
  ...
  - 早间秀 (Morning Show)：生成完整对话播客 + 插画 + 音频
  ↓
...
- `NewsPage` 重构为 News Hub，提供 Quick Read / Morning Show 模式切换
...
- [x] 角色头像在说话时显示音频波形
- [x] 统一新闻中心：首页单一"儿童新闻"入口，News Hub 页面提供 Quick Read / Morning Show 模式切换
...
- ✅ 早间秀嘉宾角色注入：通过 child_id 查询 ChromaDB 中的重复角色
- ✅ 多管线偏好更新：画作转故事、互动故事、新闻转换、早间秀均更新偏好数据
...
可插拔的多供应商 TTS 引擎，支持表情达意的语音朗读、场景风格匹配和声音设计。为所有内容管线（画作转故事、互动故事、早间秀）提供统一的高质量语音生成能力。
...
- 🔲 **前端语音选择器**: `VoicePicker` 组件，年龄适配过滤 + 语音预览（#151）
