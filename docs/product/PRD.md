...
#### 待完成 (Remaining Gaps)
- 🔲 **角色结构化存储**: 角色仅作为 ChromaDB 非结构化元数据存在，无专用表、无 CRUD、无成长追踪
- 🔲 **故事去重**: 无机制检测近似故事或注入跨故事引用（"还记得上次闪电小狗的冒险吗？"）
- 🔲 **记忆 API 暴露**: `PreferenceRepository` 仅内部使用，前端无法展示"你喜欢的主题"或角色画廊
- 🔲 **契约测试覆盖**: `PreferenceRepository` 和 `vector_search_server` 缺少契约测试
- 🔲 **隐私合规**: 偏好数据无 TTL、无删除端点、无数据最小化（COPPA 要求）
...
#### 验收标准
- [ ] `characters` SQLite 表：child_id, name, description, visual_features, traits, appearance_count, first/last_seen
- [ ] `CharacterRepository` 服务：CRUD + 按 child_id 列出角色 + 按出场次数排序
- [ ] 画作转故事和互动故事完成时自动同步角色到 `characters` 表
- [ ] ChromaDB `story_embeddings` 集合：生成前查询语义相似故事，相似度 > 0.9 触发去重
- [ ] 生成提示词注入最近 3 条故事摘要，支持跨故事引用
- [ ] `GET /api/v1/memory/preferences/{child_id}` 返回偏好数据
- [ ] `GET /api/v1/memory/characters/{child_id}` 返回角色列表
- [ ] `DELETE /api/v1/memory/preferences/{child_id}` 清除偏好数据
- [ ] `PreferenceRepository` 和 `vector_search_server` 契约测试覆盖
- [ ] `recent_choices` 上限 50 条，主题分数 6 个月未更新自动衰减

#### 不在本期范围
- 角色成长机制（特质随时间演变）— Phase 3
- 阅读难度自适应（per-child reading_level）— Phase 3
- 角色画廊前端 UI — 依赖 My Library 完成后实现
- 跨儿童角色共享
