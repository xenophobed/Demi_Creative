...
class TestStreamingProvenance:
    ...
    async def test_streaming_records_provenance_artifacts(self, sample_image, test_client):
        ...
        async with test_client as client:
            resp = await client.post("/api/v1/image-to-story/stream", files={"image": ("d.png", sample_image, "image/png")}, data={"child_id": child_id, "age_group": "6-8", "interests": "animals"})
    ...
    async def test_streaming_and_sync_record_same_artifact_types(self, sample_image, test_client):
        """Parity: streaming path records the same artifact types as sync path."""
        async with test_client as client:
            img1 = BytesIO(); Image.new("RGB", (100, 100), color="red").save(img1, format="PNG"); img1.seek(0)
            r1 = await client.post("/api/v1/image-to-story", files={"image": ("s.png", img1, "image/png")}, data={"child_id": f"c-sync-{uuid.uuid4().hex[:8]}", "age_group": "6-8"})
            assert r1.status_code == 201
            sync_id = r1.json()["story_id"]

            img2 = BytesIO(); Image.new("RGB", (100, 100), color="blue").save(img2, format="PNG"); img2.seek(0)
            r2 = await client.post("/api/v1/image-to-story/stream", files={"image": ("t.png", img2, "image/png")}, data={"child_id": f"c-stream-{uuid.uuid4().hex[:8]}", "age_group": "6-8"})
            ...
            assert await self._get_artifact_types_for_story(client, sync_id) == await self._get_artifact_types_for_story(client, stream_id)

    async def test_streaming_provenance_run_is_completed(self, sample_image, test_client):
        """Successful streaming run must mark provenance run as completed."""
        async with test_client as client:
            resp = await client.post("/api/v1/image-to-story/stream", files={"image": ("d.png", sample_image, "image/png")}, data={"child_id": f"c-done-{uuid.uuid4().hex[:8]}", "age_group": "6-8"})
            assert resp.status_code == 200
            story_id = None
            for line in resp.text.split("\n"):
                if line.startswith("data:") and "story_id" in line:
                    ...
                    if story_id:
                        break
            assert story_id is not None
            lineage = (await client.get(f"/api/v1/admin/artifacts/stories/{story_id}/lineage")).json()
