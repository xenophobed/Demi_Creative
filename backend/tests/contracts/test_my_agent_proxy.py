...
class TestSkillGating:
    ...
    async def test_start_interactive_story_records_structured_result(self):
        """The proxy keeps an in-process copy of tool results so live SDK
        versions that summarize ToolResultBlock content still emit
        launch_flow metadata."""
        sink: dict[str, Any] = {}
    ...
    async def test_start_interactive_story_persists_opening_session_for_resume(self):
        """Openings generated through My Agent must be resumable at
        /interactive?session=... just like the standalone story flow."""
        sink: dict[str, Any] = {}
        server = _build_tools_for(
            _fake_agent(enabled_skills=["interactive_story"]),
            result_sink=sink,
...
class TestLaunchFlowEvent:
    ...
    async def test_image_story_text_request_without_image_launches_upload(self, test_db):
        """A child typing "image to story" has already expressed the flow
        intent. With no image attached, the proxy should deterministically
        launch the upload page instead of hoping the SDK calls a tool."""
        events = await _collect_launch_flow_run(
            messages=[_fake_result_message("This should not be needed.")],
    ...
    async def test_kids_daily_text_request_launches_kids_daily(self, test_db):
        """The chat starter 'What's in the news today for kids?' should
        launch Kids Daily using the active child age group, not ask for age."""
        events = await _collect_launch_flow_run(
            messages=[_fake_result_message("This should not be needed.")],
