...
class VoiceSessionData:
    ...
    # Transport column (#647) — ``ws`` for the server-relay broker path,
    # ``webrtc`` for the browser-direct OpenAI handshake. Pre-#647 rows
    # carry NULL; the Phase D dashboard treats NULL as ``ws`` since that
    # was the only path before this migration landed.
...
class VoiceSessionRepository:
    ...
    async def create_session(
        ...
        *,
        ...
        ``transport`` (#647) records ``ws`` vs ``webrtc`` so the Phase D
        dashboard can split p50 latency by path. Defaults to NULL —
        callers from older code paths keep the legacy behavior.
        """
        ...
        await self._db.execute(
            ...
            INSERT INTO voice_sessions
                (session_id, user_id, child_id, started_at, ended_at,
                 ...
                 termination_reason, provider, transport)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                resolved_session_id,
