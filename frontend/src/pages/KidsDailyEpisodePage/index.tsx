...
function KidsDailyEpisodePage() {
  ...
  const progressRatio = Math.min(
    1,
    Math.max(0, currentStoryTime / totalDuration),
  );
  const trackingChildId =
    currentChild?.child_id || defaultChildId || episode?.child_id || "";
  ...
  const isFinished =
    hasStarted &&
    !isPlaying &&
    currentLineIndex >= Math.max(0, lines.length - 1);
  ...
  useEffect(() => {
    ...
    const currentAudioUrl = morningShowAudioSrc(
      episode.audio_urls[String(currentLineIndex)],
    );
                ...
                {activeIllustration ? (
                  <motion.img
                    ...
                    animate={{
                      opacity: 1,
                      scale:
                        activeIllustration.animation_type === "zoom"
                          ? 1.08
                          : 1.04,
                      x: activeIllustration.animation_type === "pan" ? -12 : 0,
                      y:
                        activeIllustration.animation_type === "ken_burns"
                          ? -8
                          : 0,
                    }}
                    exit={{ opacity: 0 }}
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm text-gray-500">
                  Line {lines.length > 0 ? currentLineIndex + 1 : 0}/
                  {lines.length}
                </div>
              </div>
              ...
              <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                <motion.div
                  ...
                  animate={{
                    width: `${Math.min(100, Math.max(0, (currentStoryTime / totalDuration) * 100))}%`,
                  }}
